...
import { ParsedStats, ParsedStatsOutput } from '../parsing/match-parser.js';
import { TeamScore } from '../stats/team-stats.js';
...
export type PlayerMapping = Record<string, number>;
...
export class DB {
    ...
    protected async query<T = void>(query: string, ...params: any[]): Promise<T[]> {
...
//  LIMIT 50;
    ...
    public async getLogJson(log_name: string): Promise<ParsedStatsOutput | undefined> {
    ...
    public async getLogPlayerJson(log_name: string, player_id: string): Promise<{ player: PlayerOutputStats; game: ParsedStatsOutput } | undefined> {
    ...
    public async getReparseLogs(reparseType: ReparseType = ReparseType.ReparseNew): Promise<ReparseMetadata[]> {
        switch (reparseType) {
            ...
            case ReparseType.ReparseAll: {
                const logs = await this.query<ReparseMetadata>(
                    'SELECT id, log_file1, log_file2 FROM logs WHERE id > 42'
                );
                ...
                await this.query('UPDATE logs SET is_valid = NULL');
    ...
    public async getUniqueLogName(parse_name: string, reparse?: boolean): Promise<string> {
        ...
        const junk = Math.random().toString(36).substr(2, 5);
    ...
    protected async ensurePlayers(client: pg.PoolClient, playerLists: PlayerList[]): Promise<PlayerMapping> {
        let players: Player[] = [];
    ...
    protected async addEvent(client: pg.PoolClient, roundId: number, event: Event): Promise<void> {
        client.query(
            `INSERT INTO 
    ...
    public async matchTransaction(
        ...
        try {
            ...
            await this.recordLogJsons(logId, playerMapping, parsedStats, chartMarkup, playerOutputStats, client);
    ...
    protected async saveRoundTeams(
    ...
    protected async saveScore(client: pg.PoolClient, score: TeamScore, logId: number): Promise<void> {
        await client.query(
            `UPDATE logs
