...
import { MapLocation } from '../models/map-location.js';
import type Player from '../models/player.js';
import { Event, type ExtraData } from '../models/event.js';
import { TeamColor, Weapon } from '../models/types.js';
import { RoundState } from '../state/round-state.js';
import { explodeLine, parseClass, parseTeam, parseWeapon } from './line-parsers/parse-helpers.js';

export function createEventFromLine(lineNumber: number, line: string, roundState: RoundState): Event | string | undefined {
    ...
    if (line[0] === 'L') {
        ...
        const lineDataParts = explodeLine(lineData);
        ...
        try {
            ...
            if (fromPlayerDataParts !== null) {
                ...
                const playerFromTeam = parseTeam(fromPlayerDataParts[4]);
                ...
                if (otherPlayerDataParts != null) { // Two players were affected.
                    ...
                    const playerToTeam = parseTeam(otherPlayerDataParts[4]);
                    ...
                    switch (nonPlayerDataParts[0]) {
                        case "killed":
                            if (nonPlayerDataParts[1] === "with") {
                                ...
                                withWeapon = parseWeapon(nonPlayerDataParts[2]);
                            } else
                        ...
                        case "triggered":
                            if (nonPlayerDataParts[1].startsWith("airshot")) {
                                ...
                                withWeapon = parseWeapon(nonPlayerDataParts[4]);
                    ...
                    switch (nonPlayerDataParts[0]) {
                        ...
                        case "joined":
                            ...
                            playerTo = playerFrom;
                            data.team = parseTeam(nonPlayerDataParts[2]);
                            break;
                        case "entered":
                        ...
                        case "changed":
                            if (nonPlayerDataParts[1] === "name") {
                                ...
                                data.class = parseClass(nonPlayerDataParts[3]);
                            }
                        ...
                        /* e.g., L 11/20/2018 - 01:54:42: "phone<59><STEAM_0:0:44791068><Blue>" committed suicide with "trigger_hurt" (world); L 11/20/2018 - 01:46:41: "pheesh-L7<64><STEAM_0:0:64178><Red>" committed suicide with "train" (world); "tomaso<19><STEAM_0:0:7561319><Blue>" committed suicide with "the red team's lasers" (world) */
                            ...
                            withWeapon = parseWeapon(weaponString);
                            break;
                        case "triggered":
                            switch (nonPlayerDataParts[1]) {
                                ...
                                case "Dispenser_Dismantle":
                                    eventType = EventType.PlayerDismantledBuilding;
                                    data.building = parseWeapon("dispenser");
                                    break;
                                ...
                                case "Teleporter_Exit_Finished":
                                    ...
                                    data.building = parseWeapon(nonPlayerDataParts[1]);
                                    break;
                ...
                switch (nonPlayerDataParts[0]) {
                    ...
                    case "World":
                        ...
                        switch (nonPlayerDataParts[2]) {
                            ...
                            case "Red Flag Returned Message":
                                ...
                                data.team = parseTeam(nonPlayerDataParts[2].split(" ")[0]);
                                break;
                    ...
                    case "Team":
                        ...
                        data.team = parseTeam(nonPlayerDataParts[1]);
                        data.value = nonPlayerDataParts[3];
                        break;
                    case "[ETANA]":
